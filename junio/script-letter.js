// --- CONFIGURACIÓN DE TU GALERÍA DE IMÁGENES DE JUNIO ---
const imagenesJunio = [
  { url: 'fotos/imagen1.png', comentario: 'Ese dia me rei mucho, fue muy divertido' },
  { url: 'fotos/imagen2.jpeg', comentario: 'Me encanta cuando haces eso' }
];
...
galleryPopup.addEventListener('click', () => {
  ...
  if (indiceActual < imagenesJunio.length) {
    cargarImagen();
  } else {
...
function cargarImagen() {
  const fotoFila = imagenesJunio[indiceActual];
  activeGalleryImg.src = fotoFila.url;
  activeGalleryComment.textContent = fotoFila.comentario;
  imgCounter.textContent = `${indiceActual + 1} / ${imagenesJunio.length}`;
}